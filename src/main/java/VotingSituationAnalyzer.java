...
public class VotingSituationAnalyzer {
    ...
    public ArrayList<VotingOption> analyzeVoter(int voterIndex) {
        ...
        generateVotingOptions(votingOptions, new boolean[candidateScores.length], new int[candidateScores.length], initialHappiness, 0, truePreferences, candidateScores, voterIndex);

        return votingOptions;
    }

    private void generateVotingOptions(ArrayList<VotingOption> bucket,
                                       boolean[] usedVoting,
                                       int[] fakePreferences,
        ...
        if (candidateIndex == candidateScores.length) {
            ...
            if (newHappiness > initialHappiness) {
                ...
                bucket.add(new VotingOption(transformedOutcome, newOutcome, newHappiness, voterIndex));
            }
        ...
        for (int i = 0; i < votingVector.length; i++) {
            ...
            generateVotingOptions(bucket, usedVoting, fakePreferences, initialHappiness, candidateIndex + 1, truePreferences, candidateScores, voterIndex);
